...
sticker_place = driver.find_element_by_xpath('//*[@id="symbolID"]')
start_date = driver.find_element_by_xpath('//*[@id="fHistoricalPrice_FromDate"]')
end_date = driver.find_element_by_xpath('//*[@id="fHistoricalPrice_ToDate"]')
sticker_place.send_keys(sticker)
start_date.send_keys(start)
end_date.send_keys(end)
WebDriverWait(driver,1)
driver.find_element_by_xpath('//*[@id="fHistoricalPrice_View"]').click()
...
while True:
    try:
      #  next_link = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "[title^=' Next to']")))
        ...
        tbl = driver.find_element_by_xpath('//*[@id="tab-1"]/div[2]/ul').get_attribute('outerHTML')
        df = pd.read_html(tbl)
        #refine data
        mdf = df[0]
        if(len(sticker) > 3): 
